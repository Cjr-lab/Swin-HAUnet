...
parser.add_argument('--output_dir', type=str,
                    default='./outputs/swin-haunet', help='output dir')
...
parser.add_argument('--test_save_dir', type=str, default='./predictions/SG-former_u',
...
def inference(args, model, test_save_path=None):
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        print("i_batch= {},case_name= {},mean_dice= {},mean_hd95= {}".format(i_batch,case_name,np.mean(metric_i, axis=0)[0],np.mean(metric_i, axis=0)[1]))
...
if __name__ == "__main__":
    ...
    # net = Sgformer_U(img_size=args.img_size,in_chans=1, num_class=args.num_classes).cuda()
    net=SwinUnet(config, img_size=args.img_size, num_classes=[args.num_classes]).cuda()
    # net =  UNet(1,n_classes=args.num_classes).cuda()
    # net = ViT_seg(config,img_size=args.img_size,in_chans=1, num_class=args.num_classes).cuda()

    snapshot = os.path.join(args.output_dir, 'best_model.pth')
    if not os.path.exists(snapshot): snapshot = snapshot.replace('best_model', 'epoch_' + str(
        args.max_epochs - 1))
    # print("---",snapshot)
    ...
    # log_folder = './test/test_SG-former'
    log_folder = './test/test_swinhaunet'
    # log_folder = './test_log/test_log_/outputs'
